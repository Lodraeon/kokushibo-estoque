# KOKUSHIBO 🌙
**K**it **O**rganizacional de **K**ardex **U**nificado: **S**istema **H**íbrido de **I**nventário e **B**anco **O**timizado.

## Sobre o Projeto
Projeto Integrador desenvolvido para o curso de Engenharia de Software. O objetivo é fornecer uma solução simples e eficiente para controle de estoque em pequenas empresas e ONGs.

## Tecnologias Utilizadas
* **Linguagem:** Python 3
* **Banco de Dados:** SQLite3
* **Interface:** CLI (Linha de Comando)

## Funcionalidades (CRUD)
* [x] **C**reate: Adicionar novos produtos.
* [x] **R**ead: Listar todos os produtos em tabela.
* [x] **U**pdate: Atualizar quantidade e preço.
* [x] **D**elete: Remover produtos do sistema.

## Como executar
1. Certifique-se de ter o Python instalado.
2. Clone este repositório ou baixe o arquivo `.py`.
3. Execute no terminal:
   ```bash
   python kokushibo_estoque.py
